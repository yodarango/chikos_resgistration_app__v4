...
pub fn Radio (props: &FormRadioProps)-> Html {
    ...
    html!{
        <div class="form_radio">
            <input type="radio" name={name.clone()} checked={checked[0]} value={value.get(0).map(|val| val.clone())} onchange={&onchange}/>
            <input type="radio" name={name.clone()} checked={checked[1]} value={value.get(1).map(|val| val.clone())} onchange={&onchange}/>
            <label>{label}</label>
        </div>
    }
}